...
def drop_postgres_user():
    ...
    try:
        ...
        cursor.execute(
            sql.SQL("DROP USER IF EXISTS {};").format(sql.Identifier(db["USER"]))
        )
    ...
    try:
        ...
        cursor.execute(
            sql.SQL("CREATE DATABASE {} OWNER {};").format(
                sql.Identifier(db["NAME"]), sql.Identifier(db["USER"])
            )
        )
...
def drop_database():
    ...
    try:
        ...
        cursor.execute(sql.SQL("DROP DATABASE {};").format(sql.Identifier(db["NAME"])))
    ...
    try:
        with engine.connect() as conn:
            with open(backup_file, "w") as file:
                for line in conn.execution_options(
                    stream_results=True
                ).connection.connection.iterdump():
                    file.write("%s\n" % line)
...
def check_user_exists(username):
    with next(get_db()) as db:
        ...
        if user:
            print(
                f"User '{username}' exists in the database with hashed password: {user.hashed_password}"
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--create-pg-user", action="store_true", help="Create the PostgreSQL user"
    )
    parser.add_argument(
        "--drop-pg-user", action="store_true", help="Drop the PostgreSQL user"
    )
    ...
    parser.add_argument(
        "--list-columns", type=str, help="List all columns in a specified table"
    )
    parser.add_argument(
        "--truncate-tables", nargs="+", help="Truncate specified tables"
    )
    parser.add_argument(
        "--backup-db", type=str, help="Backup the database to a specified file"
    )
    parser.add_argument(
        "--restore-db", type=str, help="Restore the database from a specified file"
    )
    parser.add_argument(
        "--create-user",
        action="store_true",
        help="Create a user. Add --username, --email, --password and --fullname as well.",
    )
    ...
    parser.add_argument(
        "--role", type=str, default="user", help="Role for the user (default: user)"
    )
    ...
    parser.add_argument(
        "--assign-role",
        action="store_true",
        help="Assign a role to a user. Add --username, --rolename",
    )
    parser.add_argument(
        "--rolename", type=str, help="Name of the role to assign to the user"
    )

    parser.add_argument(
        "--create-roles-and-permissions",
        action="store_true",
        help="Add endpoint permissions to database",
    )
    ...
    if args.create_user:
        if not (args.username and args.email and args.password and args.fullname):
            print(
                "Username, email, password, and full name must be provided to create a user."
            )
            sys.exit(1)
        with next(get_db()) as db:
            create_user(
                db, args.username, args.email, args.password, args.fullname, args.role
            )
    if args.create_role:
        with next(get_db()) as db:
    ...
    if args.assign_role:
        ...
        with next(get_db()) as db:
            ...
            print(
                f"Role '{args.rolename}' assigned to user '{args.username}' successfully."
            )
