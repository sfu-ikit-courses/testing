...
@router.post(
    ...
    status_code=201,
    dependencies=[Depends(permission_required("create_user"))],
)
...
@router.get(
    "/{user_id}",
    response_model=UserResponse,
    dependencies=[Depends(permission_required("read_user"))],
)
def get_single_user(user_id: int, db: Session = Depends(get_db)) -> UserResponse:
...
@router.get(
    ...
    dependencies=[Depends(permission_required("read_all_users"))],
)
...
@router.post(
    ...
    response_model=UserResponse,
    status_code=200,
    dependencies=[Depends(permission_required("assign_role_to_user"))],
)
...
@router.delete(
    ...
    response_model=UserResponse,
    status_code=200,
    dependencies=[Depends(permission_required("remove_role_from_user"))],
)
